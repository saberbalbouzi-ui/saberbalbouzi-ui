            ...
            <div className="p-4 md:p-6 space-y-4 max-h-[600px] overflow-y-auto">
              ...
              {activeCategory === 'jawami' && (
                <div className="mt-6 bg-amber-50 rounded-xl p-4 border border-amber-200">
                  <h4 className="text-amber-800 font-extrabold text-lg mb-3">أدعية الأنبياء</h4>
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center">
        ...
        <p className="text-white/90 text-lg md:text-xl lg:text-2xl leading-relaxed mb-10 max-w-3xl mx-auto">
          منصة متخصصة تجمع الرقاة الشرعيين المعتمدين في جميع الدول.
          <br />
...
function FeaturedRaqisSection() {
  const [featuredRaqis, setFeaturedRaqis] = useState<
    (Raqi & { featured_badge?: boolean })[]
  >([]);
    ...
    const loadFeatured = async () => {
      ...
      try {
        ...
        const featured = (Array.isArray(data) ? data : [])
          .filter((raqi: Raqi) => raqi.featured_badge)
          .slice(0, 12);
  ...
  const getCityLabel = (raqi: Raqi) => {
    if (!raqi.wilaya) return '';

    // القيمة المخزنة حالياً في raqis.wilaya نصية
    return raqi.wilaya;
  };
    ...
    return (
      <section className="bg-[#0b5a35] px-4 py-8">
        <div className="max-w-5xl mx-auto flex items-center justify-center py-6">
          ...
          <span className="mr-3 text-white/80 font-semibold">
            جاري التحميل...
          </span>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-white/75 max-w-2xl mx-auto leading-relaxed">
            مجموعة مختارة من الرقاة أصحاب الشارة المميزة.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {featuredRaqis.map((raqi) => (
            ...
            >
              <Card className="rounded-2xl border border-white/10 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 overflow-hidden bg-white">
                <div className="w-full aspect-square bg-[#f3f7f4] overflow-hidden">
                  {raqi.profile_image_url ? (
                    <img
                      src={raqi.profile_image_url}
                      alt={`صورة ${raqi.full_name}`}
                      className="w-full h-full object-cover"
                      loading="lazy"
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
                      <User className="w-12 h-12 text-[#1f6f50]" />
                    </div>
                  )}
                </div>
                ...
                <div className="p-5 flex flex-col gap-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 text-gray-600">
                      <Globe className="w-4 h-4 text-[#1f6f50] shrink-0" />
                      <span>
                        {getCountryLabel(raqi.country_code)}
                      </span>
                    </div>
...
function SearchSection() {
  ...
  const handleSearch = () => {
    ...
    navigate(`/roqat?${params.toString()}`);
  };
    ...
    <section className="bg-[#0b5a35] py-12 px-4">
      <div className="max-w-3xl mx-auto">
        <p className="text-center text-white text-lg font-semibold mb-8">
          يرجى اختيار الدولة والولاية لعرض النتائج.
        </p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                className="w-full h-12 px-4 border border-gray-200 rounded-xl text-base bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#1f6f50]"
              >
                <option value="">
                  {countriesLoading
                    ? 'جاري تحميل الدول...'
            ...
            <Button
              ...
              className="w-full h-12 bg-gradient-to-br from-[#1f6f50] to-[#15523b] hover:from-[#18593f] text-white font-bold rounded-xl shadow-lg transition-all hover:-translate-y-0.5"
            >
      ...
      <div className="max-w-2xl mx-auto text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Button
            ...
            className="w-full sm:w-auto min-w-[250px] h-14 bg-gradient-to-br from-[#1f8f57] to-[#2dbf68] hover:from-[#1a7a49] hover:to-[#25a85a] text-white text-lg font-extrabold rounded-xl shadow-xl transition-all hover:-translate-y-0.5"
          >
  ...
  return (
    <footer className="bg-[#083d24] py-8 px-4 border-t border-white/5">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="text-white/50 text-sm mb-2">منصة متخصصة للرقاة الشرعيين المعتمدين.</p>

        <p className="text-white/30 text-xs">© {new Date().getFullYear()} جميع الحقوق محفوظة</p>
      </div>
    </footer>
  );
}
